...
acf(ipcLag12.ts, lag =20)
```

No es estacionaria por que los distinto meses y años no se comportan igual, ya que se observan años con medias positivas y otros con medias negativas (e.g 2007 (7) y 2009 (9)) en la serie temporal ***ipcLag12.ts** y por ende con cambios en las varianzas*.
...
**• The IPC series after having taken consecutively differences with lags 1 (regular difference) and 12 (seasonal difference). Plot this series previously: diff( diff(IPC,lag=1), lag=12). Do you think it is stationary?**

```{r}
par(mfrow = c(3,3), mar = c(rep(3,4)))
...
# ----------------------------------
test = diff( diff(ipcIndex.ts,lag=1), lag=12)
acf(test, main = "test")
