...
#Comparison
plot(forecast(best.arima.IPC.tr, h=15),col="red", lw=5, main = "ARIMA(1,1,0)(0,1,1)[12] vs IPC.te")
lines(ipcIndex.ts,col="green", lw=0.1)
```
...
plot(forecast(IPC.tr.HW, h=15),col="blue", lw=5, main = "Holt-Winters vs IPC.te")
lines(ipcIndex.ts,col="green", lw=0.1)
```
...
Por otra parte, la predicción de HW suaviza la curva más que el modelo ARIMA, y éste modelo realizó una predicción con mayor precisión de la tendencia y los picos de la curva al seguir el mismo patron que los valores de IPC.te.
