...
llpinar1 = function(par,deaths.Sunday.ts){
  T = length(deaths.Sunday.ts)
  value = -log(dpois(deaths.Sunday.ts[1], par[1]/(1-par[2])))
  for(t in c(2:T)) {
	  value = value-log(tppois(deaths.Sunday.ts[t], deaths.Sunday.ts[t-1], par[1], par[2]))
  }
value
}
